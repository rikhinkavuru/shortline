...
Start: `shortline mcp` (stdio). Set `SHORTLINE_DB` to the same database the dashboard and CLI use.
...
| `shortline_plan_find` | no | `watch_id?`, `product?`, `region`, `need?`, `wave_size?`, `max_waves?`, `ask_hold?`, `near?` | `find_request_id`, known sources, ranked candidates (masked), skipped sites with reasons, `estimated_calls` |
...
`shortline_run_find` blocks until the request finishes (waves complete one at a time). In live mode a wave of three takes a few minutes.
