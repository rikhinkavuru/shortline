...
Shortline is a runnable app (`apps/typescript/shortline` in this repository) that treats CALL-E calls as measurements. This skill drives it through its MCP server or CLI. It never dials on its own.
...
1. **Connect.** Start `shortline mcp` (stdio) from the app directory, or use the CLI. In dry-run mode (the default) every call is simulated; say so to the user.
...
3. **Plan, never dial.** Call `shortline_plan_find` with the watch or product, the region, and how many confirmed sources are needed (default 2). Show the user:
   - known sources from surveillance in the last 24 hours (no call needed);
   - the ranked candidates with **masked** numbers and the estimated number of calls;
...
- Never guess a phone number, region, or timezone. Sites come from the operator's list.
