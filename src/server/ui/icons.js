// Lucide-style line icons as inline SVG strings. Every icon is decorative
// (aria-hidden); the surrounding element carries the accessible name.
...
const PATHS = {
  ...
  "circle-slash": '<line x1="9" x2="15" y1="15" y2="9"/><circle cx="12" cy="12" r="10"/>',
...
/** Render one icon. `name` must be a key of PATHS; unknown names fall back to a dashed circle. */
export function icon(name, { size = 16, cls = "" } = {}) {
  const body = PATHS[name] ?? PATHS["circle-dashed"];
  const s = Number(size) || 16;
  const className = cls ? `icon ${cls}` : "icon";
  return `<svg class="${className}" width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${body}</svg>`;
}
